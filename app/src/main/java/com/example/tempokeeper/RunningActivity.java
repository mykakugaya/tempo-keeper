...
public class RunningActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    // Handler to keep updating the run duration (updates the hrs:mins:secs)
    ...
    // check the pace average every 30 paces to determine if there needs to be a song change
    private final int RATE_PEDOMETER_CHECK = 30;
    // the maximum bpm difference is used to compare the new average pace value to the previous
    // and change the song if the updated pace is more than last average +20 or -20
    // Range: target BPM plus 10 or minus 10
    private final int MAX_BPM_DIFF = 20;
    ...
    private double lastSignificantAverage;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        txtRunningDist = (TextView) findViewById(R.id.txtRunningDist);
        txtRunningDur = (TextView) findViewById(R.id.txtRunningDur);

        txtRunningDist.setText("Distance: ");
        txtRunningDur.setText("Duration: ");
        ...
        // if coming from the route form activity, get the target distance
        // notify user when they reach the target distance
        try {
            targetDistance = getIntent().getExtras().getDouble("targetDist");
        } catch (Exception e) {
            targetDistance = 0;
        }
        ...
        paceValues = new int[RATE_PEDOMETER_CHECK];   // stores the last 30 pace readings in an array
        paceValuesIndex = 0;    // pace values index ranges from 0-29
        lastPaceAverage = 0;
        lastSignificantAverage = 0;
        resetValues();
        ...
        Snackbar snackbar = Snackbar.make(btnPause, "Click \"Start Run\" to start your run!", Snackbar.LENGTH_LONG);
        ...
        btnFinish.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                // saves the run to database - route and other stats info
                saveRouteToDb();
                ...
                // Notify user that run is finished
                Snackbar snackbar = Snackbar.make(btnPause, "You have finished your run!", Snackbar.LENGTH_SHORT);
                snackbar.getView().setBackgroundColor(R.color.colorPrimaryDark);
                snackbar.show();
        ...
        btnStats.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                finishIntent.putExtra("runDur",duration);
                finishIntent.putExtra("runDist",totalDistance.toString());
                finishIntent.putExtra("runAvg",averageSpeed.toString());
    ...
    public void onMapReady(@NonNull GoogleMap googleMap) {
        ...
        //enable Start button after Map is ready
        ...
        btnStart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // start updating the run duration every second
                setRunDuration();
    ...
    //Functioned called every second from LocationRequest on Create
    ...
    private void setUserLocation(Location location) {
        ...
        if (running == true){
            if (runningRoute.size()>1){
                ...
                txtRunningDist.setText("Distance: "+totalDistance.toString()+" miles");
                // if the user has reached their target distance (from route form),
                // send a snackbar telling user that they have reached it
                if (targetDistance != 0 && totalDistance >= targetDistance && !targetDistReached){
        ...
        CameraPosition cameraPosition = new CameraPosition.Builder()
                .target(latLng)                     //Camera centered on user
                .zoom(19)                           // Sets the zoom
                .bearing(location.getBearing())     // Sets the orientation of the camera to always be pointed up
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    ...
                    if (Integer.valueOf(temp)>0) {  // if past runs exist, append this run
                        ...
                        double hour = duration_ms/MS_TO_HR;
                        averageSpeed = round((round(totalDistance,4)/hour), 2);
                        avgSpdLst[avgSpdLst.length-1] = String.valueOf(averageSpeed);
                        ...
                        double hour = duration_ms/MS_TO_HR;
                        averageSpeed = round((round(totalDistance,4)/hour), 2);
                        dbRef.child(firebaseUser).child("Average").setValue(Arrays.asList(averageSpeed));
    ...
    private StepService.ICallback mCallback = new StepService.ICallback() {
        ...
        */
        ...
        public void paceChanged(int value) {
            // use a Handler to send/process the message
    ...
    */
    ...
    // This is the handler that handles messages coming from the step service interface methods
    ...
    public void addToPaceArray(int pace) {
        ...
        if(paceValuesIndex == RATE_PEDOMETER_CHECK - 1) {
            int sum = 0;
            for (int value : paceValues) {
                sum += value;
            }
            // Calculate the average
            double averagePace = Math.round(sum/RATE_PEDOMETER_CHECK);

            // Compare this new average pace value to the last average pace value
            // If the two values have a difference of more than 20 bpm,
            // we will trigger a change in track BPM and change the remotely playing track
            if(averagePace > lastSignificantAverage + MAX_BPM_DIFF || averagePace < lastSignificantAverage - MAX_BPM_DIFF) {
    ...
    private void dynamicTrackSelection(double curTempo) {
        ...
        if(curTempo <= 0) {
            ...
            if(filteredTracks.size() > 0) {
                ...
                // Make sure that the random song selected from filteredTracks is not the same
                // as the currently playing song
                while(nextTrack == null || nextTrack.getName().equals(sharedPreferences.getString("curTrack",""))) {
                    ...
                    nextTrack = filteredTracks.get(randomIndex);
                }

                // Play new song with updated tempo
                playbackService.play(nextTrack);
                // Show a snackbar notifying the user of track change
            ...
            // start playing a random song from the entire playlist
            else {
                getAllTracks(); // playlistTracks has all playlist tracks
                // Pretty much same as if statement, but select a random index in playlistTracks and get that track
                Track nextTrack = null;
                while(nextTrack == null || nextTrack.getName().equals(sharedPreferences.getString("curTrack",""))) {
                    Random rand = new Random();
                    int index = rand.nextInt(playlistTracks.size());
                    nextTrack = playlistTracks.get(index);
                }
                // Play the new track
                playbackService.play(nextTrack);
                // Show a snackbar notifying the user of track change
            ...
            /* Since we change songs dynamically, we want to update the track holder every second to
            * see any track/tempo changes, as well as playback progress
            */
    ...
    public void setCurTrackInfo() {
        ...
        if(!trackName.equals("")) {
            ...
            // We have a Track object, display on PlayingTrackAdapter and set seekbar progress
            if (newTrack != null) {
    ...
    // Thread to update the run duration (hrs:mins:secs) every second
    private void setRunDuration() {
        runHandler = new Handler();
        Runnable r = new Runnable() {
            @RequiresApi(api = Build.VERSION_CODES.N)
            public void run() {
                long curDuration = System.currentTimeMillis() - startTime;
                String strDuration;
                if(curDuration < 3600000) { // duration is less than an hr
                    // convert ms to mins:secs
                    int minutes = (int)(curDuration / 1000) / 60;
                    int seconds = (int)(curDuration / 1000) % 60;
                    if (seconds >= 10) {
                        strDuration = minutes + ":" + seconds;
                    } else {
                        strDuration = minutes + ":0" + seconds;
                    }
                } else {    // duration exceeds an hour
                    // convert ms to hrs:mins:secs
                    int seconds = (int) (curDuration / 1000) % 60 ;
                    int minutes = (int) ((curDuration / (1000*60)) % 60);
                    int hours = (int) ((curDuration / (1000*60*60)) % 24);
                    String strMins = String.valueOf(minutes);
                    String strSecs = String.valueOf(seconds);
                    if (seconds < 10) {
                        strSecs = "0"+strSecs;
                    ...
                    strDuration = hours+":"+strMins+":"+strSecs;
                }
                txtRunningDur.setText("Duration: "+strDuration);
